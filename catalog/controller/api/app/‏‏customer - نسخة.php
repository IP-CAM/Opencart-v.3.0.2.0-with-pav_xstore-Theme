...
class ControllerApiAppCustomer extends Controller {
	...
	public function returnCustomerJson($succes_message = 'Success !' , $is_update = false)
	{
					...
					$userDetails[] = array(
						...
						'customers_default_address_id' => $this->customer->getAddressId(),//TODO
						'liked_products' => $liked_products,
						'is_number_verify' => 0, //TODO
					);
					$is_update == false ?
	...
	public function updateCustomerInfo() {
		...
		else
		{
			...
			if($data['customers_id'] == $this->customer->getId()){
				$data['customers_id'] = $data['customers_id'] ;
			}else
				$json['error']['customers_id'] = true ;
			...
			if (!$json) {
				...
				$this->model_account_customer->editCustomer($data['customers_id'] , $data);
	...
	{	
		//$this->customer->logout(); //DEBUG
		//var_dump($this->customer->isLogged()) ;die();  //DEBUG

		// if user logged in
		if(isset($this->session->data['customer']) && $this->session->data['customer'] ){
			//try to logged in
